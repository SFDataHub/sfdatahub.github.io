...
export function AnchoredLineChart({
  ...
  const { linePath, areaPath, dots, avgY, labels } = useMemo(() => {
    if (!points.length) {
      return { linePath: "", areaPath: "", dots: [] as { x: number; y: number }[], avgY: null as number | null, labels: [] as { x: number; text: string }[] };
    }
    ...
    const scaleX = (idx: number) =>
      points.length > 1 ? PLOT_LEFT + (idx / (points.length - 1)) * plotWidth : PLOT_LEFT;
    const scaleY = (v: number) => clamp(PLOT_TOP + (1 - (v - min) / range) * plotHeight, PLOT_TOP, PLOT_BOTTOM);
    ...
    const coords = points.map((p, idx) => ({ x: scaleX(idx), y: scaleY(p) }));
    ...
    const labels = points.map((_, idx) => ({ x: scaleX(idx), text: `t${idx}` }));
