...
const GuideHubV2: React.FC = () => {
  ...
  const handleNavigate = React.useCallback(
    (next: { tab: string; sub?: string; sub2?: string }) => {
          ...
          if (hasDeeper) {
                  ...
                  <div key={`${cat.key}-${subItem.key}-${s2.key}`} className={styles.subGroup}>
                    ...
                    {leaves.map((s3) => (
                      <button
                        ...
                        onClick={() => handleNavigate({ tab: cat.key, sub: subItem.key, sub2: s3.key })}
                        type="button"
                      >
                        {s3.label}
                      </button>
            ...
            leafBlocks = leaves.map((s2) => (
              <button
                ...
                onClick={() => handleNavigate({ tab: cat.key, sub: subItem.key, sub2: s2.key })}
                type="button"
              >
                {s2.label}
              </button>
        ...
        return (
          <div key={`${cat.key}-${subItem.key}`} className={styles.subSection}>
            <button
              ...
              onClick={() => {
                handleToggleSub(cat.key, subItem.key);
                handleNavigateSub(cat.key, subItem.key);
              }}
      ...
      return (
        <div key={cat.key} className={styles.navSection}>
          <button
            ...
            onClick={() => {
              handleToggleTab(cat.key);
              handleNavigateCategory(cat.key);
            }}
            ...
            {doc && viewStatus !== "missing" && (
              <>
                ...
                <div className={contentGridClass}>
                  <div className={markdownClass} dangerouslySetInnerHTML={{ __html: doc.html }} />
