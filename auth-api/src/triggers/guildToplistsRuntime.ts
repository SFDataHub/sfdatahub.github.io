...
const SNAPSHOT_SUFFIX = "_guild_derived";
...
const buildContentSignature = (data?: Record<string, any>): string => {
  ...
  const pick = (idx: number) => {
    const row = guilds[idx];
    if (!row || typeof row !== "object") return "";
    const id = String((row as any).guildId ?? (row as any).id ?? (row as any).name ?? "");
    const sumAvg = (row as any).sumAvg ?? (row as any).avgSumBaseTotal ?? (row as any).sum ?? "";
    return `${id}:${sumAvg}`;
  };
...
export async function handlePublishGuildLatestToplists(event: any) {
  ...
  if (!serverCode) {
    console.warn("[publishGuildLatestToplists] Unable to resolve server code", {
  ...
  const publicRef = db.doc(
    `stats_public/toplists_guilds_v1/lists/latest_toplists/servers/${serverCode}`,
  );
  await publicRef.set(
    {
      server: serverCode,
